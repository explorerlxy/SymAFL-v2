...
def run(mode, trace_skip_depth=-1):
    r, w = os.pipe()
    ...
    env = dict(os.environ)
    skip_opt = f":trace_skip_depth={trace_skip_depth}"
    if mode == "direct":
        env["TAINT_OPTIONS"] = (f'taint_file="{SEEDS}/seed1":pipe_fd={w}:'
                                f'exit_on_memerror=false{skip_opt}')
        ...
        env["TAINT_OPTIONS"] = (f'taint_file="{out}/default/.cur_input":'
                                f'taint_max_len=65536:pipe_fd={w}:'
                                f'exit_on_memerror=false{skip_opt}')
    ...
    print(f"[{mode}] skip_depth={trace_skip_depth} bytes={len(buf)} cond={n_cond} gep={n_gep} memcmp={n_memcmp} "
...
if __name__ == "__main__":
    ...
    trace_skip_depth = -1
    if "--skip-depth" in sys.argv:
        try:
            skip_arg = sys.argv.index("--skip-depth")
            trace_skip_depth = int(sys.argv[skip_arg + 1])
        except (IndexError, ValueError):
            print("usage: trace_check.py [direct|afl] [--skip-depth K]", file=sys.stderr)
            sys.exit(2)
    sys.exit(0 if run(mode, trace_skip_depth) else 1)
